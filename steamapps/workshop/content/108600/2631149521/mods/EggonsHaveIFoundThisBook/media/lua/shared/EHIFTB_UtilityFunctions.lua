...
EHIFTB.isItemMemorized = function(item)
    local memory = EHIFTB.getPlayerMemory()
    local identifier = EHIFTB.getItemIdentifier(item)
    if memory.rememberedBooks[identifier] then
        return memory.rememberedBooks[identifier]
    elseif memory.redundantBooks[identifier] then
        return "redundant"
    end
    return false
end
