...
-- This file overwrites the one with the same name from the gamefiles to add the required functionality.
...
--Added function to check the snow too
...
function SRainBarrelSystem:checkSnow()
	...
	for i=1,self:getLuaObjectCount() do
		...
		if luaObject.waterAmount < luaObject.waterMax then
			...
			if luaObject.exterior then
				luaObject.waterAmount = math.min(luaObject.waterMax, luaObject.waterAmount + (1 * RainCollectorBarrel.waterScale * snowscaler)/100) --snow water is half of what should be with rain
				luaObject.taintedWater = true
				local isoObject = luaObject:getIsoObject()
				if isoObject then -- object might have been destroyed
