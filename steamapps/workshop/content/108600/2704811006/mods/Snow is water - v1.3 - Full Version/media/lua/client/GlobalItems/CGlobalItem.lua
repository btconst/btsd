...
CGlobalItem = ISBaseObject:derive("CGlobalItem")

function CGlobalItem:noise(message)
	self.luaSystem:noise(message)
end

function CGlobalItem:new(luaSystem, globalObject)
	...
	-- Override initNew() to initialize a brand-new CGlobalItem.
...
function CGlobalItem:getIsoObject()
...
function CGlobalItem:getSquare()
	return getCell():getGridSquare(self.x, self.y, self.z)
end

function CGlobalItem:fromModData(modData)
...
function CGlobalItem:updateFromIsoObject()
