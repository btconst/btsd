...
CGlobalItemSystem = ISBaseObject:derive("CGlobalItemSystem")

function CGlobalItemSystem:noise(message)
	if self.wantNoise then print('C ' .. self.systemName..': '..message) end
end

function CGlobalItemSystem:new(name)
...
function CGlobalItemSystem:initSystem()
end

function CGlobalItemSystem:initLuaObjects()
...
function CGlobalItemSystem:getLuaObjectCount()
	return self.system:getObjectCount()
end

function CGlobalItemSystem:getLuaObjectByIndex(index)
	return self.system:getObjectByIndex(index-1):getModData()
end

function CGlobalItemSystem:isValidIsoObject(isoObject)
	error "override this method"
end

function CGlobalItemSystem:getIsoObjectOnSquare(square)
...
function CGlobalItemSystem:getIsoObjectAt(x, y, z)
	local square = getCell():getGridSquare(x, y, z)
	return self:getIsoObjectOnSquare(square)
end

function CGlobalItemSystem:newLuaObject(globalObject)
	-- Return an object derived from CGlobalItem
	error "override this method"
end

function CGlobalItemSystem:newLuaObjectAt(x, y, z)
	local globalObject = self.system:newObject(x, y, z)
	return self:newLuaObject(globalObject)
end

function CGlobalItemSystem:removeLuaObject(luaObject)
...
function CGlobalItemSystem:removeLuaObjectAt(x, y, z)
	local luaObject = self:getLuaObjectAt(x, y, z)
	self:removeLuaObject(luaObject)
end

function CGlobalItemSystem:getLuaObjectAt(x, y, z)
...
function CGlobalItemSystem:getLuaObjectOnSquare(square)
	if not square then return nil end
	return self:getLuaObjectAt(square:getX(), square:getY(), square:getZ())
end

function CGlobalItemSystem:OnLuaObjectUpdated(luaObject)
	-- luaObject fields were updated with new values from the server
	self:noise('OnLuaObjectUpdated')
end

function CGlobalItemSystem:sendCommand(playerObj, command, args)
	self.system:sendCommand(command, playerObj, args)
end

function CGlobalItemSystem:OnServerCommand(command, args)
...
function CGlobalItemSystem.RegisterSystemClass(luaClass)
	if luaClass == CGlobalItemSystem then error "replace : with . before RegisterSystemClass" end
